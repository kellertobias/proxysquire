...
console.log("[Proxysquire] Loaded. Ready for injecting require stubs.");
var path = require('path')
  , Module = require('module')
  , resolve = require('resolve')
  , dirname = require('path').dirname
  , ProxysquireError = require('./proxysquire-error')
  , is = require('./is')
...
function validateArguments(request, stubs) {
  ...
  if (msg) throw new ProxysquireError(msg);
}

function Proxysquire(parent) {
  var self = this
    , fn = self.load.bind(self)
    , proto = Proxysquire.prototype
    ;
 ...
 * @function
 * @private
 * @return {object} The proxysquire function to allow chaining
 */
Proxysquire.prototype.noCallThru = function () {
 ...
 * @return {object} The proxysquire function to allow chaining
 */
Proxysquire.prototype.callThru = function () {
  this._noCallThru = false;
  return this.fn;
};

/**
 * Will make proxysquire remove the requested modules from the `require.cache` in order to force 
 * them to be reloaded the next time they are proxysquired.
 ...
 * @return {object} The proxysquire function to allow chaining
 */
Proxysquire.prototype.noPreserveCache = function() {
  this._preserveCache = false;
  return this.fn;
};

/**
 * Restores proxysquire caching behavior to match the one of nodejs `require` 
 *
 * @name preserveCache
 * @function
 * @private
 * @return {object} The proxysquire function to allow chaining
 */
Proxysquire.prototype.preserveCache = function() {
  this._preserveCache = true;
  return this.fn;
};
 ...
 */
Proxysquire.prototype.load = function (request, stubs, b, verbose) {
  this.__pqbasepath = b; // Kind of hackish...
  this.__pqverbose = !!verbose; // Definitely hackish...
  if (this.__pqverbose) console.log("require", request);
  ...
  for (var key in stubs) {
    ...
    if (typeof stub === 'undefined') {
      throw new ProxysquireError('Invalid stub: "' + key + '" cannot be undefined');
    }
  ...
  return this._withoutCache(this._parent, stubs, request, this._parent.require.bind(this._parent, this.__pqbasepath + request));
};

// This replaces a module's require function
Proxysquire.prototype._require = function(module, stubs, path) {
  assert(typeof path === 'string', 'path must be a string');
  assert(path, 'missing path');
  if(module.id.indexOf('node_modules') < 0) {
    var requiringModule = "" + module.id;
    requiringModule = requiringModule.substr(this.__pqbasepath.length);
    if (this.__pqverbose) console.log("     ->", requiringModule, hasOwnProperty.call(stubs, path)?"[STUBBED]":"", path);
  }
  ...
  var loadPath = path
  if(loadPath.indexOf('/') == 0 && loadPath.indexOf(this.__pqbasepath) < 0) {
    loadPath = this.__pqbasepath + loadPath
  }
  // Only ignore the cache if we have global stubs
  if (this._containsRuntimeGlobal) {
    return this._withoutCache(module, stubs, path, Module._load.bind(Module, loadPath, module));
  } else {
    return Module._load(loadPath, module);
  }
};

Proxysquire.prototype._withoutCache = function(module, stubs, path, func) {
  // Temporarily disable the cache - either per-module or globally if we have global stubs
  var restoreCache = this._disableCache(module, this.__pqbasepath + path);
  ...
  try {
    ...
    if (this.__pqverbose) console.log("Proxysquire Require Error: ", e);
  }
   finally {
...
Proxysquire.prototype._disableCache = function(module, path) {
...
Proxysquire.prototype._disableGlobalCache = function() {
...
Proxysquire.prototype._disableModuleCache = function(path, module) {
...
Proxysquire.prototype._overrideExtensionHandlers = function(module, stubs) {
